...
void Storage::PopulateLayout(DataLayout &layout)
{
    ...
    // load maneuver overrides
    {
        ...
        layout.SetBlock(DataLayout::MANEUVER_OVERRIDES, make_block<extractor::StorageManeuverOverride>(number_of_overrides));
        const auto number_of_nodes = maneuver_overrides_file.ReadVectorSize<NodeID>();
        layout.SetBlock(DataLayout::MANEUVER_OVERRIDE_NODE_SEQUENCES, make_block<NodeID>(number_of_nodes));
    }

    {
        ...
        if (boost::filesystem::exists(config.GetPath(".osrm.mldgr")))
        {
            ...
            layout.SetBlock(DataLayout::MLD_GRAPH_NODE_LIST, make_block<customizer::MultiLevelEdgeBasedGraph::NodeArrayEntry>(num_nodes));
            layout.SetBlock(DataLayout::MLD_GRAPH_EDGE_LIST, make_block<customizer::MultiLevelEdgeBasedGraph::EdgeArrayEntry>(num_edges));
            layout.SetBlock(DataLayout::MLD_GRAPH_NODE_TO_OFFSET, make_block<customizer::MultiLevelEdgeBasedGraph::EdgeOffset>(num_node_offsets));
        }
        else
        {
            layout.SetBlock(DataLayout::MLD_GRAPH_NODE_LIST, make_block<customizer::MultiLevelEdgeBasedGraph::NodeArrayEntry>(0));
            layout.SetBlock(DataLayout::MLD_GRAPH_EDGE_LIST, make_block<customizer::MultiLevelEdgeBasedGraph::EdgeArrayEntry>(0));
            layout.SetBlock(DataLayout::MLD_GRAPH_NODE_TO_OFFSET, make_block<customizer::MultiLevelEdgeBasedGraph::EdgeOffset>(0));
        }
    }
}
